...
<div class="TopListItem">
  <div>
    <div class="TopListTitle">
      {{ list.name }}<span class="iconfont icon-daohangyou"></span>
    </div>
    <ul>
      <li v-for="(item, index) in musicList" :key="index">
        <a href="">
          <div>
            <img alt="" v-lazy="item.picUrl" /><span class="iconfont icon-bofang"></span>
          </div>
...
export default {
  ...
  created() {
    gettoplist(this.list.id).then(res => {
      let path = res.playlist ? res.playlist.tracks : 0;
      if (res.playlist) {
        for (let i = 0; i < 3; i++) {
          this.musicList.push({
            id: path[i].al.id, // id
